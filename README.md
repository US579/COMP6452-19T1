...
* Problem
   ...
   * Solution
      ...
      *  Factory contract is deployed once from off-chain source code
        问题: 这句话的意思就是smart contract的模板source code代码库只用deploy一次,只要确保这一次不出问题,代码on-cahin了以后,用smart contract来call smart contract来创建实例就解决了上面的问题
...
3. Advantages of MDE in the blockchain context(要考)
...
* fork
   * soft fork 
      * 升级协议,新版本的节点发布,所有节点都可以验证通过(新旧),但旧版本发布的版本只能在旧版本节点通过,就会产生分叉,随着用户的升级,分叉会逐渐减小,最终成为一条链
   * hard fork
      * 制定了新的协议,新旧版本只能各自版本的链上进行验证,从而伸展出来两条链,这会两条链永远不会重合
